...
const LinechartContent = () => {
  ...
  const calculatePreviousDayTotal = () => {
    ...
    const previousMonthData = data.month.find(monthData => monthData.year === previousYear)?.months.find(month => new Date(`${month.month} 1, 
    ${previousYear}`).getMonth() + 1 === previousMonth);

    const previousDayData = previousMonthData?.daily_data.find(dayData => parseInt(dayData.date.split('-')[2], 10) === previousDay);
  ...
  const processData = () => {
    ...
    if (granularity === 'day') {
      processedData = data.month
        ...
        .map(({ date, amount_sold, amount_delivered, amount_ordered }) => ({
          date: date.slice(8),
          amount_sold,
          amount_delivered,
          amount_ordered
      ...
      processedData = data.month
        ...
        .flatMap(({ months }) =>
          months.map(({ month, monthly_totals }) => ({
            ...
            amount_delivered: monthly_totals.amount_delivered,
            amount_ordered: monthly_totals.amount_ordered
          }))
        );
      ...
      processedData = data.month.map(({ year, months }) => ({
        ...
        amount_delivered: months.reduce(
          (acc, { monthly_totals }) => acc + monthly_totals.amount_delivered,
          0
        ),
        amount_ordered: months.reduce(
          (acc, { monthly_totals }) => acc + monthly_totals.amount_ordered,
          0
        )
      }));
    }
  ...
  return (
    ...
    >
      ...
      />
      <LineChart width={1112} height={400} data={flattenedData}>
