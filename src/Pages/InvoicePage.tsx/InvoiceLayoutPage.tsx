...
import { ApiResponse, LineItemType, useCreateGCCMutation, useInitGCCMutation } from '@/Redux/Features/Gcc/Gcc';
...
const InvoiceLayoutPage: React.FC = () => {
  ...
  const [gccData, setGccData] = useState<ApiResponse['data'] | null>(null);
  const [showDateInfo, setShowDateInfo] = useState<boolean>(false);

  const [initGCC] = useInitGCCMutation();
  const [createGCC] = useCreateGCCMutation();
  

  useEffect(() => {
    const fetchGcc = async () => {
      const response = await initGCC({ customer_id: 31, customer_site_id: 11 });

      if (response?.data?.data?.list_item?.length) {
        setGccData(response.data.data);
      ...
      if (!response?.data?.data?.gcc || !response?.data?.data?.list_item?.length) {
        setShowDateInfo(false);
      }
    };
  ...
  const tabs = [
    {
      ...
      component: () => (
        <InventoryTable
          onCreateInvoiceAdviceClick={() => setShowProceed(true)}
          onGccCreate={handleCreateGCC}
        />
      ),
    },
    {
    ...
    },
    {
      name: 'Invoice Advice',
      component: () => (
        gccData?.invoice_advice ? <InvoiceAdvice /> : null
      ),
    },
    {
      name: 'Invoice',
